...
get_series <- function(table_px, index, ...){
  ...
  if(frequency(series_ts)==4){
    ...
    fig1 <- plot_ly(as.data.frame(series_obj@data), x = seq(from=start(series_obj@data)[1],by=0.25,length.out=length(series_obj@data[,1])), y = series_obj@data[,"level"], type = 'scatter', mode = 'lines', name = paste0('Level', " (",table_px$UNITS$value,")"))
  }
  if(frequency(series_ts)==1){
    ...
    fig1 <- plot_ly(as.data.frame(series_obj@data), x = seq(from=start(series_obj@data)[1],by=1,length.out=length(series_obj@data[,1])), y = series_obj@data[,"level"], type = 'scatter', mode = 'lines', name = paste0('Level', " (",table_px$UNITS$value,")"))
  }
  if(frequency(series_ts)==12){
  ...
  fig1 <- fig1 %>% layout(title = list(text = chart_title, xanchor = 'center', yanchor = 'top', font = list(family = "Times New Roman", size = 12, color = "black")))
  fig1 <- fig1 %>% layout(xaxis = list(tickformat = ",.", hoverformat = ",."))
  print(fig1)
